...
namespace UtilSharpDX.DrawingCommand.Default
{
    public abstract class DrawingEffectEx : MCDrawingEffect
    {
        ...
        public const int W_SIZE = 1024;
        public const int H_SIZE = 1024;
        public const string g_spliteHLSL = "sprite.hlsl";
        public const string g_D3dGameHLSL = "D3dGame.hlsl";
        ...
        /// テクスチャーカメラハンドル
        /// </summary>
        protected MCBaseCamera m_hTxCamera;
        /// <summary>
        /// 一時的に保存するレンダーターゲット
        /// </summary>
        ...
        /// 
        /// </summary>
        protected DefaultDrawSpriteRegister m_spriteRegister;
        ...
        {
            m_spriteRegister = new DefaultDrawSpriteRegister(app);
        }
        ...
        protected int CreateTextureCamera(string name, int w = 1024, int h = 1024)
        {
            ...
            if (!App.CameraMgr.GetCamera(name, out m_hTxCamera))
            {
                ...
                if (!MCTextureCamera.Create(App, name, w, h, out m_hTxCamera)) return -1;
                MCTextureCamera hTmp = (MCTextureCamera)m_hTxCamera;
                if (hTmp is null) return -1;
        ...
        protected int CreateViewTextureCamera(string name)
        {
            ...
            if (App.CameraMgr.GetCamera(name, out m_hTxCamera))
            {
                // 名前をつけて”カメラタイプ１”を作成しハンドルを取得する。
                var v = App.SwapChainDesc.ModeDescription;
                if (!MCTextureCamera.Create(App, name, v.Width, v.Height, out m_hTxCamera)) return -1;
                MCTextureCamera hTmp = (MCTextureCamera)m_hTxCamera;
                if (hTmp is null) return -1;
